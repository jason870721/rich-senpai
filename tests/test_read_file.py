...
def test_offset_only_reads_to_end():
    with tempfile.TemporaryDirectory() as tmp:
        f = _write(Path(tmp) / "test.txt", _lines(10))
        result = read_file(str(f), offset=8)
        assert result.ok
...
def test_negative_offset_clamped_to_one():
    with tempfile.TemporaryDirectory() as tmp:
        f = _write(Path(tmp) / "test.txt", _lines(5))
        result = read_file(str(f), offset=-5, limit=2)
        assert result.ok
        assert "showing lines 1-2" in result.text
...
def test_single_line_slice():
    with tempfile.TemporaryDirectory() as tmp:
        f = _write(Path(tmp) / "test.txt", _lines(5))
        result = read_file(str(f), offset=3, limit=1)
        assert result.ok
...
def test_file_not_found():
    result = read_file("/tmp/__nonexistent_rich_senpai_test__")
